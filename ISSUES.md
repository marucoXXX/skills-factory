...
**Status**: 進行中（Phase 1: 4 SKILL.md skeleton 作成中） / **Priority**: P2 / **Decided**: 2026-04-27 / **Re-scoped**: 2026-04-29
...
### 新規作成スキル（4 件）
1. `business-overview-pptx` — 事業の概要 1 枚スライド（PPTX 単体）
2. `company-deepdive-agent` — 会社レベル 5 論点 + セグメント検出 + 事業深掘り N 回呼ぶ + merge
3. `business-deepdive-agent` — 事業セグメント単位 5 論点
4. `comparison-synthesis-agent` — 全社比較 + 検証論点統合
...
### 上場・非上場の境界
v0.3 では境界判定を入れず、**取れる公開情報から最大限作成**。取れなかった項目は `data-availability-pptx` で「✗未取得」明示。
...
- ユーザー提示 v4.0 元仕様: `/Users/nakamaru/Desktop/BDD_SKILLS_DEVELOP/files/README.md`
