...
**Status**: 進行中（Phase 2-4 + 後追補完了、Phase 5 のみ残） / **Priority**: P2 / **Decided**: 2026-04-27 / **Re-scoped**: 2026-04-29 / **Updated**: 2026-04-29
...
4. ⏳ `comparison-synthesis-agent` — 全社比較 + 検証論点統合 — **Phase 5 で実装予定**
...
| Phase 5 | comparison-synthesis-agent 実装 | ⏳ 未着手 | 3-5 社の company-deepdive-agent 出力を入力に横並び比較・検証論点統合デッキ生成 |
...
### Phase 5 の概要（次セッション以降で着手）
- 3〜5 社の `company-deepdive-agent` 出力（PPTX + segment_summary.json）を入力
- 横並び比較サマリー + 検証論点統合デッキを生成
- `competitor-summary-pptx/SKILL.md` の `competitor-analysis-pptx` 参照を `comparison-synthesis-agent` に書き換え
- Phase 4 後追補で導入した `cross_topic_consistency_check.md` を本スキルからも参照取り込みする
- 所要 1-2 日
