"""One-shot generator: pyramid-structure-pptx の roleup curated template を
...
設計:
  ...
  - Pyramid Diagram Area: A4 縮小版に再配置
...
SRC = os.path.join(SKILL_DIR, "assets", "stellar_aiz", "pyramid-structure-template.pptx")
DST = os.path.join(SKILL_DIR, "assets", "roleup", "pyramid-structure-template.pptx")
...
def main():
    ...
    diagram = _find(slide, "Pyramid Diagram Area")
    if diagram is not None:
        diagram.left = Inches(0.30)
        diagram.top = Inches(1.55)
        diagram.width = Inches(11.10)
        diagram.height = Inches(5.70)
