...
## 必須裏取りクレームタイプ（scope=high_risk で省略不可）

`scope=high_risk` 以上で実行する場合、以下のクレームタイプは **必ず WebSearch で裏取りすること**。
代表数件のみのサンプリングで済ませるショートカットは禁止。

| カテゴリ | 必須裏取り対象 | 推奨ソース |
|---------|--------------|-----------|
| **A. 上場企業の財務** | 売上高・営業利益・経常利益・純利益（直近期）| EDINET / 決算短信 PDF / 有価証券報告書 |
| **B. 業界市場規模・成長率** | 市場規模（億円・兆円）・CAGR・将来予測値 | 研究機関名指し（矢野経済・ミック経済研究所・富士経済 等）／業界団体統計 |
| **C. 主要プレイヤーの顧客社数・利用者数** | 「導入XX社」「ユーザーXX万人」等の社数・人数 | 公式プレスリリース／公式HPの「数字で見る」セクション |
| **D. 市場シェア%** | 「シェアXX%」と記載のあるすべての数値 | 原典（有償レポート）か推計か必ずタグ付け。原典名と公表年を明示 |
| **E. 政府政策の金額・施行年** | 補助金額・予算額・法令施行年 | 経済産業省／内閣府／首相官邸／関連省庁の公式ページ |

**運用ルール**:
- 各カテゴリのクレームは抽出時に `mandatory_verification: true` フラグを付与（`extract_claims.py` で実装）
- Step 2 の WebSearch 実行時、`mandatory_verification=true` のクレームを WebSearch せずに `not_yet_verified` のまま残すことは **禁止**
- 時間制約等で全件裏取りが難しい場合も、上記 A〜E のクレームは最優先で消化する
- 非上場プレイヤーの売上推計（A の派生）は `single_source` または `not_found` 扱いとし、出典の限界を必ず注記
...
- **裏取りに使う検索回数**は claim 1件あたり 1〜2 回まで。無駄な検索は避ける（ただし上記必須裏取りカテゴリは省略不可）
