...
## 6. ブランド別 palette の扱い（V1 以降）

### 6.1 stella の chart_palette

本ファイル §1〜§4 が **stella の正本** として継続。
`market-environment-pptx` は brand-aware 化済（`fill_market_environment.py` の `CHART_PALETTE` 等は brand=stellar_aiz 時に theme.json から resolve され、本ファイルと完全一致するよう同期維持）。
...
将来 stella の `chart_palette.md` も `_common/brands/stellar_aiz/theme.json` に統合可能（V2+ 検討）。

### 6.3 ロールバック

§6 を削除すれば §1〜§5 不変で原状回帰可能。brand-aware 化は本ファイルの既存運用を破壊しない。
