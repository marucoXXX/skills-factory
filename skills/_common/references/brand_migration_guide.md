...
を brand-aware 化し、`stellar_aiz` / `rollup` の出力切替機構を確立した。
...
```
scope.json (brand: "rollup")
    ↓ orchestrator が読む
fill_*.py --brand rollup
    ↓
brand_resolver.resolve_brand("rollup", SKILL_DIR)
    ├─ skills/_common/brands/rollup/theme.json     (色・フォント・チャートパレット)
    └─ skills/<skill>/assets/rollup/layout.json    (skill 固有座標)
...
```bash
mkdir -p skills/<skill>-pptx/assets/{stellar_aiz,rollup}
...
`assets/rollup/layout.json`: V1 placeholder（stella と同値、curated rollup テンプレ未配置時はフォールバック）
...
| rollup の chart_palette 同期源 | — | `_common/brands/rollup/theme.json` (1 ファイル、自動 resolve) |
| 同期負荷 | 5 ファイル × 編集ごと | 0（rollup は theme.json 単一） |

**rollup 以降の新ブランドは theme.json 単一源**。stella の手動同期負荷は新ブランドに伝播しない。
...
Rollup 仕様の `Yu Gothic UI` は macOS 標準搭載されない。3 段階の対策：
...
### (C) Windows 環境で最終チェック
Rollup 納品前の最終視覚レビューは Windows / Office for Mac で行う。

**Deliverable PPTX の中身は `Yu Gothic UI` 指定のまま固定**（Rollup 社内 Windows 環境で正しく表示）。
...
- [ ] `assets/{stellar_aiz,rollup}/layout.json` 作成
- [ ] regression-zero 確認: `--brand` 未指定 / `stellar_aiz` 明示の出力が現状と diff 無し（random axId 等は許容）
- [ ] `--brand rollup` で例外なく完走（fill 生成箇所が rollup 色 / Yu Gothic UI に切替わる）
- [ ] SKILL.md に `--brand` を文書化、アセット表更新

### V2 で curated rollup template を入れる際の追加作業
- [ ] PowerPoint で A4 横（11.69×8.27）の Rollup テンプレを作成、shape 名を stella と合わせる
- [ ] `assets/rollup/<skill>-template.pptx` に配置
- [ ] `assets/rollup/layout.json` を A4 横向け座標に更新
