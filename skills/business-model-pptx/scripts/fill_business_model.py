...
# ── Theme-controlled module variables ──────────────────────
# Defaults match stella for safety; reassigned in main() via _apply_theme(theme)
FONT_NAME_JP = "Noto Sans CJK JP, Noto Sans JP, Meiryo UI"
COLOR_TEXT = RGBColor(0x33, 0x33, 0x33)

# CSS hex strings (no '#') for HTML CSS injection
COMPANY_BG_HEX  = "1A365D"   # company / business-line emphasis (stella default)
ENTITY_BG_HEX   = "EBF4FF"   # supplier / customer box bg (stella default)
ENTITY_BORDER_HEX = "4A5568" # entity box border (stella default)
ENTITY_NAME_HEX = "1A365D"   # entity name color (stella default)
ENTITY_DESC_HEX = "4A5568"   # entity desc color (stella default)
ARROW_PROVIDE_HEX = "C53030" # right-direction arrow (provide flow, stella default)
ARROW_PAYMENT_HEX = "2B6CB0" # left-direction arrow (payment flow, stella default)
COMPANY_DESC_HEX  = "BEE3F8" # company desc color (light) — fixed, not brand-driven
BIZ_METRICS_HEX   = "90CDF4" # biz-line metrics text — fixed, not brand-driven
HEADER_LABEL_HEX  = "1A365D" # multi-biz header label color
TEXT_HEX          = "333333"


def _apply_theme(theme):
    """Override module-level CSS hex / fonts from theme.json."""
    global FONT_NAME_JP, COLOR_TEXT, TEXT_HEX
    global COMPANY_BG_HEX, ENTITY_BG_HEX, ENTITY_BORDER_HEX
    global ENTITY_NAME_HEX, ENTITY_DESC_HEX
    global ARROW_PROVIDE_HEX, ARROW_PAYMENT_HEX, HEADER_LABEL_HEX
    FONT_NAME_JP = theme.font_ea
    COLOR_TEXT = theme.color("text")
    TEXT_HEX = theme.hex_no_hash("text")
    COMPANY_BG_HEX = theme.hex_no_hash("label_bar")          # company/biz emphasis
    ENTITY_BG_HEX = theme.hex_no_hash("label_bg")            # entity box bg
    ENTITY_BORDER_HEX = theme.hex_no_hash("source")          # gray-tone border
    ENTITY_NAME_HEX = theme.hex_no_hash("label_bar")         # entity name
    ENTITY_DESC_HEX = theme.hex_no_hash("source")            # entity desc
    ARROW_PROVIDE_HEX = theme.hex_no_hash("highlight_target") # provide flow
    ARROW_PAYMENT_HEX = theme.hex_no_hash("accent_op_margin_line") # payment flow
    HEADER_LABEL_HEX = theme.hex_no_hash("label_bar")        # multi-biz header
...
def _apply_run_style(run):
    """Apply brand font + text color to a python-pptx run."""
    run.font.name = FONT_NAME_JP
    run.font.color.rgb = COLOR_TEXT


def set_placeholder_text(shape, text):
    """PlaceholderのTextFrameにテキストをセット（既存スタイルを保持しつつ font/color のみ brand 化）"""
    if shape is None:
        return
    tf = shape.text_frame
...
def fill_implications(slide, items):
    ...
    既存テンプレート rPr を継承しつつ font/color のみ brand 化する。
    """
    ...
    for i, item in enumerate(items[:3]):
        ...
        def _make_run(text_str, bold=False):
            ...
            if template_rPr is not None:
                ...
                solidFill = new_rPr.find(qn("a:solidFill"))
                if solidFill is not None:
                    new_rPr.remove(solidFill)
                ...
                if bold:
                    attrib["b"] = "1"
                rPr = etree.SubElement(r_elem, qn("a:rPr"), attrib=attrib)
...
def _generate_html_multi(data, company, suppliers, customers, businesses):
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    body {{
        font-family: '{FONT_NAME_JP}', 'Noto Sans CJK JP', 'Noto Sans JP', 'Meiryo UI', sans-serif;
        background: white;
    ...
    .biz-box {{
        border: 2px solid #{COMPANY_BG_HEX};
        border-radius: 5px;
        padding: {sz['biz_pad']};
        background: #{COMPANY_BG_HEX};
    ...
    .biz-desc {{
        ...
        color: #{COMPANY_DESC_HEX};
        line-height: 1.3;
    }}
    .biz-metrics {{
        font-size: {sz['biz_desc']}px;
        color: #{BIZ_METRICS_HEX};
        line-height: 1.3;
        margin-top: 2px;
    }}

    .entity-box {{
        border: 2px solid #{ENTITY_BORDER_HEX};
        border-radius: 6px;
        padding: {sz['box_pad']};
        text-align: center; width: 100%; height: 100%;
        background: #{ENTITY_BG_HEX};
        display: flex; flex-direction: column;
    ...
    .entity-name {{
        ...
        color: #{ENTITY_NAME_HEX};
        margin-bottom: 3px; line-height: 1.3;
        word-break: keep-all; overflow-wrap: break-word;
    }}
    .entity-desc {{
        font-size: {sz['entity_desc']}px;
        color: #{ENTITY_DESC_HEX}; line-height: 1.3;
    }}
    ...
    .arrow-label-provide {{ color: #{ARROW_PROVIDE_HEX}; }}
    .arrow-label-payment {{ color: #{ARROW_PAYMENT_HEX}; }}
    .arrow-line {{
        flex: 1 0 20px; height: 2px; position: relative;
    }}
    .arrow-line-right {{ background: #{ARROW_PROVIDE_HEX}; }}
    .arrow-line-right::after {{
        ...
        border-left: {sz['arrow_head']+2}px solid #{ARROW_PROVIDE_HEX};
    }}
    .arrow-line-left {{ background: #{ARROW_PAYMENT_HEX}; }}
    .arrow-line-left::before {{
        ...
        border-top: {sz['arrow_head']}px solid transparent;
        border-bottom: {sz['arrow_head']}px solid transparent;
        border-right: {sz['arrow_head']+2}px solid #{ARROW_PAYMENT_HEX};
    }}
    .empty-cell {{ }}
    """
...
def _css_common(sz, max_rows):
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .entity-box {{
        border: 2px solid #{ENTITY_BORDER_HEX}; border-radius: 6px;
        padding: {sz['box_pad']}; text-align: center;
        width: 100%; height: 100%; background: #{ENTITY_BG_HEX};
        display: flex; flex-direction: column;
        justify-content: center; align-items: center;
    }}
    .entity-name {{
        font-weight: bold; font-size: {sz['entity_name']}px;
        color: #{ENTITY_NAME_HEX}; margin-bottom: 4px; line-height: 1.3;
        word-break: keep-all; overflow-wrap: break-word;
    }}
    .entity-desc {{
        font-size: {sz['entity_desc']}px; color: #{ENTITY_DESC_HEX}; line-height: 1.3;
    }}
    .arrow-cell {{
        display: flex; flex-direction: column;
        align-items: stretch; justify-content: center;
...
def _css_company_single(sz, max_rows):
    ...
    .company-box {{
        border: 3px solid #{COMPANY_BG_HEX}; border-radius: 8px;
        padding: {sz['company_pad']}; text-align: center;
        width: 100%; height: 100%; background: #{COMPANY_BG_HEX}; color: white;
        display: flex; flex-direction: column;
        justify-content: center; align-items: center;
    ...
    .company-desc {{
        font-size: {sz['company_desc']}px; color: #{COMPANY_DESC_HEX}; line-height: 1.4;
    }}
    """
...
async def take_screenshot(html_content, output_path, vp_width, vp_height):
...
def main():
    parser = argparse.ArgumentParser(
        description="事業モデルデータをPPTXに流し込む。--brand stellar_aiz / roleup を選択 "
                    "(--template 未指定時は brand_resolver.template_path で自動解決)。",
    )
    parser.add_argument("--data",     required=True, help="business_model_data.json のパス")
    parser.add_argument("--template", required=False, default=None, help="テンプレートPPTX (任意)")
    ...
    _apply_theme(theme)
    template_path = args.template or theme.template_path(SKILL_DIR, "business-model")
    ...
    validate_fill_input(
        ...
        allowed_top=[
            ...
            "company", "businesses", "suppliers", "customers", "implications",
            "title", "subtitle",
        ],
        skill_name=SKILL_ID,
    )

    # ── PPTX 読み込み（Rectangle 4 サイズから viewport を逆算）─────
    prs = Presentation(template_path)
    slide = prs.slides[0]
    rect_shape = find_shape(slide, SHAPE_DIAGRAM_AREA)

    if rect_shape:
        # Rectangle 4 のサイズを inch 換算 → ×200 DPI で viewport 算出
        diagram_w_in = rect_shape.width / 914400.0
        diagram_h_in = rect_shape.height / 914400.0
        vp_w = int(diagram_w_in * 200)
        vp_h = int(diagram_h_in * 200)
    else:
        vp_w, vp_h = 1766, 1044

    # ── HTML生成 → スクリーンショット ──────────────────────
    print("📐 Generating business model diagram HTML...")
    html_content, html_vw, html_vh = generate_html(data)

    screenshot_path = os.path.join(tempfile.gettempdir(), "bm_diagram.png")
    print(f"📸 Taking screenshot with Playwright (viewport: {html_vw}x{html_vh})...")
    asyncio.run(take_screenshot(html_content, screenshot_path, html_vw, html_vh))
