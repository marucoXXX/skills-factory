...
- **作業ディレクトリ**: `{{WORK_DIR}}/`（= `{{FACTORY_ROOT}}/work/visual-quality-reviewer/`）
...
| 項目 | 形式 | 必須 | 説明 |
|---|---|---|---|
| `merged_pptx` | ファイルパス | 必須 | レビュー対象の PPTX（通常は `StrategyReport_*.pptx`） |
| `merge_order` | JSONパス | 任意 | Step 最終-1 で作成される `merge_order.json`（スライド番号 ↔ ファイル名の対応表） |
| `data_dir` | ディレクトリ | 任意 | `data_NN_*.json` 群の配置場所。与えられた場合、スライド番号とJSONを紐づけてコンテキストに含める |
...
LibreOffice で PPTX → PDF → PNG（150 DPI, スライドごと `page_NN.png`）に変換する。
